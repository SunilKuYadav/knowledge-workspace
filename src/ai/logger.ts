...
export function logInput(prompt: string, model?: string): void {
  ...
  if (typeof window !== 'undefined') {
    ...
    // Server environment
    console.log(`${LOG_PREFIX} [INPUT] [${timestamp}] model=${model ?? 'unknown'} prompt=${JSON.stringify(prompt)}`);
  }
}
...
export function logOutput(prompt: string, fullResponse: string): void {
  ...
  if (typeof window !== 'undefined') {
    ...
    console.log('%cResponse length:', 'font-weight: bold', fullResponse.length, 'chars');
    console.groupEnd();
  } else {
    // Server environment
    console.log(
  ...
  evtSource.onmessage = (event) => {
    try {
      ...
      if (data.type === 'chunk') {
        ...
        if (currentStreamPrompt !== data.prompt) {
          ...
          if (streamBuffer) {
            ...
            console.log('%c⬆ Prompt:', 'color: #2196F3; font-weight: bold', currentStreamPrompt);
            console.log('%c⬇ Response:', 'color: #4CAF50; font-weight: bold', streamBuffer);
            console.groupEnd();
          }
          streamBuffer = '';
      ...
      if (data.type === 'complete') {
        ...
        console.log('%c⬆ Prompt sent to model:', 'color: #2196F3; font-weight: bold');
