...
describe('FileTopicRepository', () => {
  ...
  describe('create', () => {
    it('creates topic folder with topic.json and all template markdown files', async () => {
      ...
      const overview = await readFile(path.join(folderPath, 'overview.md'), 'utf-8');
      ...
      const patterns = await readFile(path.join(folderPath, 'patterns.md'), 'utf-8');
      expect(patterns).toContain('# Binary Trees - Patterns');

      const mistakes = await readFile(path.join(folderPath, 'mistakes.md'), 'utf-8');
  ...
  describe('update', () => {
    ...
    it('updates the updatedAt timestamp', async () => {
      ...
      const updated = await repo.update('binary-trees', { status: 'completed' });
  ...
  describe('getContent and saveContent', () => {
    ...
    it('saves and reads content', async () => {
      await repo.create(sampleTopicInput);
      await repo.saveContent('binary-trees', 'notes', '# Updated Notes\n\nSome content.');
