...
function getReadinessBadge(readiness: ScoringReport['readiness']) {
  switch (readiness) {
    case 'not ready':
      return { label: 'Not Ready', className: 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400' };
    case 'needs practice':
      return { label: 'Needs Practice', className: 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-400' };
    case 'almost ready':
      return { label: 'Almost Ready', className: 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400' };
    case 'ready':
      return { label: 'Ready', className: 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' };
  }
}
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-6 text-center">
        <div className="text-5xl font-bold text-zinc-900 dark:text-zinc-100 mb-2">
          {report.overallScore}
          <span className="text-lg font-normal text-zinc-400 dark:text-zinc-500">/100</span>
        </div>
        <span className={`inline-block text-sm font-medium px-3 py-1 rounded-full ${badge.className}`}>
          {badge.label}
        </span>
