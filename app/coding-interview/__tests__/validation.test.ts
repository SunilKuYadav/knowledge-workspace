...
import type { InterviewModuleProps, InterviewSource, InterviewContext } from '../lib/types';
  ...
  describe('Property 1: Config validation', () => {
    it('valid source with matching context returns valid=true', () => {
      const arbValidProps: fc.Arbitrary<InterviewModuleProps> = fc.oneof(
        ...
        fc.constantFrom('self-test' as InterviewSource, 'practice' as InterviewSource, 'interview' as InterviewSource).chain((src) =>
          fc.record({
            source: fc.constant(src),
            context: fc.record({ source: fc.constant(src as 'self-test' | 'practice' | 'interview') }),
          })
        ),
    ...
    it('invalid source (not in valid set) returns valid=false', () => {
      const arbInvalidSource = fc.string().filter(
        (s) => !VALID_SOURCES.includes(s as InterviewSource)
      );
    ...
    it('valid source with wrong context type returns valid=false', () => {
      ...
      const arbMismatchedContext = fc.constantFrom(...VALID_SOURCES).chain((src) => {
