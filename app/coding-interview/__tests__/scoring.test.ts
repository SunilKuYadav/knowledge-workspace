...
describe("scoring.ts pure functions", () => {
  ...
  describe("applyPenalties", () => {
    it("subtracts total penalties from base score", () => {
      const result = applyPenalties(80, {
        hintsUsed: 10,
        timePenalty: 10,
        executionAttempts: 5,
      });
    ...
    it("clamps result to 0 minimum", () => {
      const result = applyPenalties(10, {
        hintsUsed: 20,
        timePenalty: 10,
        executionAttempts: 5,
      });
    ...
    it("clamps result to 100 maximum", () => {
      const result = applyPenalties(100, {
        hintsUsed: 0,
        timePenalty: 0,
        executionAttempts: 0,
      });
    ...
    it("returns integer result", () => {
      const result = applyPenalties(75.7, {
        hintsUsed: 5,
        timePenalty: 0,
        executionAttempts: 0,
      });
      ...
      fc.assert(
        fc.property(
          ...
          (
            ...
            const penaltiesA = calculatePenalty(
              hintsA,
              execAttempts,
              elapsed,
              duration,
            );
            const penaltiesB = calculatePenalty(
              hintsB,
              execAttempts,
              elapsed,
              duration,
