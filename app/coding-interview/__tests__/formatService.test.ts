  ...
  describe('Property 5: Format idempotence', () => {
    it('format(format(code)).formatted === format(code).formatted for valid code', () => {
      ...
      const arbValidCode = fc.oneof(
        ...
        fc.tuple(
          fc.string({ minLength: 1, maxLength: 10 }).map((n) => n.replace(/[^a-zA-Z]/g, 'f') || 'f'),
          fc.string({ minLength: 1, maxLength: 30 }).map((body) => body.replace(/[{}()\[\]`'"]/g, ' '))
    ...
    it('unbalanced braces → returns original code + non-empty error', () => {
      ...
      const arbUnbalancedCode = fc.oneof(
        ...
        fc.string({ minLength: 0, maxLength: 20 }).map(
          (prefix) => `${prefix.replace(/[{}]/g, 'x')}\n}`
        ),
        // Unmatched opening paren
        fc.string({ minLength: 1, maxLength: 20 }).map(
          (content) => `const x = (${content.replace(/[()]/g, 'x')}`
        ),
