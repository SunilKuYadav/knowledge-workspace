...
function isValidSource(source: unknown): source is InterviewSource {
  return (
    ...
    VALID_SOURCES.includes(source as InterviewSource)
  );
}
  ...
  switch (source) {
    case "problem": {
      ...
      if (ctx.source !== "problem") {
        return {
          valid: false,
          error: 'Context source must match prop source "problem"',
        };
      }
      if (typeof ctx.id !== "string" || ctx.id.length === 0) {
        return {
          valid: false,
          error: 'Problem context requires a non-empty "id" string',
        };
      }
      ...
      if (!Array.isArray(ctx.tags)) {
        return {
          valid: false,
          error: 'Problem context requires a "tags" array',
        };
      }
      return { valid: true };
    }
    ...
    case "topic": {
      ...
      if (ctx.source !== "topic") {
        return {
          valid: false,
          error: 'Context source must match prop source "topic"',
        };
      }
      if (typeof ctx.id !== "string" || ctx.id.length === 0) {
        return {
          valid: false,
          error: 'Topic context requires a non-empty "id" string',
        };
      }
      if (typeof ctx.title !== "string" || ctx.title.length === 0) {
        return {
          valid: false,
          error: 'Topic context requires a non-empty "title" string',
        };
      }
      if (!Array.isArray(ctx.concepts)) {
        return {
          valid: false,
          error: 'Topic context requires a "concepts" array',
        };
      }
      return { valid: true };
    }
    ...
    case "revision": {
      ...
      if (ctx.source !== "revision") {
        return {
          valid: false,
          error: 'Context source must match prop source "revision"',
        };
      }
      if (typeof ctx.sessionId !== "string" || ctx.sessionId.length === 0) {
        return {
          valid: false,
          error: 'Revision context requires a non-empty "sessionId" string',
        };
      }
      if (!Array.isArray(ctx.topicIds)) {
        return {
          valid: false,
          error: 'Revision context requires a "topicIds" array',
        };
      }
      return { valid: true };
    }
    ...
    case "interview": {
      ...
      if (ctx.source !== source) {
        return {
          valid: false,
          error: `Context source must match prop source "${source}"`,
        };
      }
      return { valid: true };
    }
...
export function validateInterviewProps(
  ...
  if (!isValidSource(props.source)) {
    return {
      valid: false,
      error: `Invalid source: "${props.source}". Must be one of: ${VALID_SOURCES.join(", ")}`,
    };
  }
  ...
  if (
    ...
    !["easy", "medium", "hard"].includes(props.difficulty)
  ) {
    return {
      valid: false,
